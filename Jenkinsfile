pipeline {
    ...
    stages {
        ...
        stage('Build image') {
            steps {
                script {
                    app = docker.build("class-mino-01/kf-img")
                }
            }
        }
        ...
        stage('K8S Manifest Update') {
            ...
            steps {
                ...
                sh "sed -i 's/kf-img:.*\$/kf-img:${env.BUILD_NUMBER}/g' kf-deployment/deployment.yaml"
                sh "git add kf-deployment/deployment.yaml"
                sh "git commit -m '[UPDATE] kf-img ${env.BUILD_NUMBER} image versioning'"
